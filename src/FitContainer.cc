...
FitContainer::FitContainer(const std::string& outputDir) :
      ...
      fullRangeId_("full_range"), // considers the range of the input data
...
// constructor from TreeContainer containing the data tree
         ...
         : FitContainer(outputDir)
{
   ...
   RooRealVar mbb(mbb_.c_str(), "M_{12}", 0.0, (*container.data()).GetMaximum(mbb_.c_str()), "GeV");
   ...
   RooDataSet dataContainer(data_.c_str(), data_.c_str(), RooArgSet(mbb, weight), RooFit::Import(*container.data()), RooFit::WeightVar(weight_.c_str()));
   workspace_.import(dataContainer);
   
}


// constructor from data histogram
FitContainer::FitContainer(const TH1* data, const std::string& outputDir, const std::string & type) :
      FitContainer(outputDir)
{
   RooRealVar mbb(mbb_.c_str(), "M_{12}", data->GetXaxis()->GetXmin(), data->GetXaxis()->GetXmax(), "GeV");
   fitRangeMin_ = mbb.getMin();
   fitRangeMax_ = mbb.getMax();
   workspace_.import(mbb);
      ...
      FitContainer(outputDir)
{
   ...
   RooRealVar mbb(mbb_.c_str(), "M_{12}", xmin, xmax, "GeV");
   fitRangeMin_ = mbb.getMin();
   fitRangeMax_ = mbb.getMax();
   workspace_.import(mbb);
...
// constructor from data tree for unbinned fit
FitContainer::FitContainer(TTree& data, const std::string& outputDir, const std::string& dataleaf, const std::string& weightleaf) :
      FitContainer(outputDir)
{
   ...
   bkg_ = "";
   signal_ = "";
   RooRealVar mbb(mbb_.c_str(), "M_{12}", 0.0, data.GetMaximum(mbb_.c_str()), "GeV");
   fitRangeMin_ = mbb.getMin();
   fitRangeMax_ = mbb.getMax();
   nbins_        = 100;
   RooRealVar weight(weight_.c_str(), "weight", 0.0, 1000.0);
   ...
   // Name and title of the dataset MUST be identical (see initialize() method).
   RooDataSet dataContainer(data_.c_str(), data_.c_str(), RooArgSet(mbb, weight), RooFit::Import(data), RooFit::WeightVar(weight_.c_str()));
   workspace_.import(dataContainer);
...
void FitContainer::initialize() {
   ...
   modelFit_.Branch("normChi2", &normChi2BkgOnly_, "normChi2/F");
   modelFit_.Branch("ndf", &ndfBkgOnly_, "ndf/I");
   modelFit_.Branch("covMatrix", covMatrix_, "covMatrix[400]/D");
   modelFit_.Branch("eigenVector", eigenVector_, "eigenVector[400]/D");
...
std::unique_ptr<RooFitResult> FitContainer::modelFit(const std::string& name, const bool & weighted, const bool& plot_params)
{
   if (!initialized_) initialize();

   auto &Pdf      = *workspace_.pdf(type_.c_str());
   ...
   // Split Range for blinded data ( and perform simultaneous fit by CA - to be implemented later R.W.)
   std::unique_ptr<RooFitResult> fitResult(Pdf.fitTo(data, RooFit::Save(),
                                                           RooFit::PrintLevel(verbosity_),
                                                           RooFit::Range(fitRangeId_.c_str()),
                                                           RooFit::SumW2Error(weighted),
                                                           RooFit::SplitRange(false)
   ));
   ...
   // Top frame
   std::unique_ptr<RooPlot> frame(mbb.frame());
   data.plotOn(frame.get(), RooFit::MarkerSize(0.8),   //0.8 for lowM
                            RooFit::DataError(RooAbsData::Auto), 
                            RooFit::Name("data_curve")
   );
   Pdf.plotOn(frame.get(),  RooFit::LineColor(kRed),
...
// Set minimum value for the fit range; modifies what is set in the contructor
...
// Set maximum value for the fit range; modifies what is set in the contructor
...
int FitContainer::getNonZeroBins_(const RooAbsData& data)
{
   ...
   for (int i = 1; i <= hist->GetNbinsX(); ++i)
   {
      ...
      if (hist->GetBinContent(i) > 0 && center > fitRangeMin_ && center < fitRangeMax_) 
         ++nonZeroBins;
   }
...
int FitContainer::getBlindedBins_(const RooAbsData& data, double blind_lowEdge, double blind_highEdge)
{
   ...
   {
      double center = hist->GetBinCenter(i);
      if (hist->GetBinContent(i) > 0. && center > blind_lowEdge && center < blind_highEdge)
         ++blindedBins;
   }
...
bool FitContainer::applyModifiers_(RooAbsPdf& pdf, const std::vector<ParamModifier>& modifiers)
{
  bool modified = false;
  RooRealVar& mbb = *workspace_.var(mbb_.c_str());
