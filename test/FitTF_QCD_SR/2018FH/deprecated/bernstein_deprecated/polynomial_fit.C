...
int polynomial_fit(const bool & mc = false, const float & xmin_ = 270, const float & xmax_ = 2000, const float & binning_ = 1, const int & poldegree_ = 1)
{
    
    cout<<"======================================================================================================================"<<endl;
    cout<<"======================================================================================================================"<<endl;
    cout<<"Fitting range "<<xmin_<<" to "<<xmax_<<" with "<<poldegree_<<"-degree polynomial, binning = "<<binning_<<endl<<endl;
    TH1::SetDefaultSumw2();
    // variable
    RooRealVar x("x", "x", xmin_, xmax_);
    // shape parameters
    RooRealVar pol_c1("pol_c1", "1st deg term", 4.5270e-07 , -0.000001, 0.000001);
    RooRealVar pol_c2("pol_c2", "2nd deg term", 7.1742e-07 , -0.000001, 0.000001);
    RooRealVar pol_c3("pol_c3", "3rd deg term", 7.1742e-07 , -0.000001, 0.000001);
    // construct the PDF

    RooAbsPdf* pol = nullptr; // Declare a pointer to the RooAbsPdf

    if (poldegree_ == 1)
    pol = new RooBernstein("bernstein", "Bernstein PDF", x, RooArgList(pol_c1, pol_c2));

    else if (poldegree_ == 2)
    pol = new RooBernstein("bernstein", "Bernstein PDF", x, RooArgList(pol_c1, pol_c2, pol_c3));
    ...
    pol_fit->Print("V");
    c_dh_ratio->SaveAs(Form("fitTF_%.0fto%.0f_bin%.0f_x0-0_pol-g%d.png", xmin_, xmax_, binning_, poldegree_ ));
    c_dh_ratio->SaveAs(Form("fitTF_%.0fto%.0f_bin%.0f_x0-0_pol-g%d.pdf", xmin_, xmax_, binning_, poldegree_ ));
