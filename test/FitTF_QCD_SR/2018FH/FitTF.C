...
int FitTF(const bool & mc = true, const float & xmin_ = 270, const float & xmax_ = 2000, const float & binning_ = 50, const int & poldegree_ = 1, const std::string & functname_ = "Chebychev", const float & x0_ = 0.)
{
    ...
    TFile *f_mc = new TFile("/nfs/dust/cms/user/leyvaped/Analyses/MSSM/FullRun2/Combine/April_2023/CMSSW_11_3_4/src/Analysis/Models/test/FitTF_QCD_SR/FinalTF/transfer_factors/with_online_btag_sf/btag_weight/mssmhbb_fh_2018_bweight_sr_x_cr_norebin_m_jet12.root", "old");
    ...
    TPaveText* text;
    if(xmax_ != 2000)
    text = new TPaveText(0.7, 0.2, 0.86, 0.4, "NDC");
    else
    text = new TPaveText(0.12, 0.65, 0.28, 0.85, "NDC");
    text->SetFillColor(0);
    ...
    funct_fit->Print("V");
    c_dh_ratio->SaveAs(Form("TFparam_2018FH/fitTF_%.0fto%.0f_bin%.0f_x0-0_%s-pol-g%d.png", xmin_, xmax_, binning_, functname_.c_str(), poldegree_ ));
    c_dh_ratio->SaveAs(Form("TFparam_2018FH/fitTF_%.0fto%.0f_bin%.0f_x0-0_%s-pol-g%d.pdf", xmin_, xmax_, binning_, functname_.c_str(), poldegree_ ));
