...
int polynomial_fit(const bool & mc = true, const float & xmin_ = 270, const float & xmax_ = 2000, const float & binning_ = 1, const int & poldegree_ = 1, const std::string & functname_ = "Chebychev", const float & x0_ = 0.)
{
    ...
    RooRealVar pol_cheb_d1_c1("pol_cheb_d1_c1", "1st deg term", 0.1 , -1, 1);

    RooRealVar pol_cheb_d2_c1("pol_cheb_d2_c1", "1st deg term 2nd order pol", 3.3209e-01 , -1, 1);
    RooRealVar pol_cheb_d2_c2("pol_cheb_d2_c2", "2nd deg term 2nd order pol",  0 , -0.1, 0.1);
    ...
    pol_fit->Print("V");
    c_dh_ratio->SaveAs(Form("TFparam_2017FH/fitTF_%.0fto%.0f_bin%.0f_x0-0_%s-pol-g%d.png", xmin_, xmax_, binning_, functname_.c_str(), poldegree_ ));
    c_dh_ratio->SaveAs(Form("TFparam_2017FH/fitTF_%.0fto%.0f_bin%.0f_x0-0_%s-pol-g%d.pdf", xmin_, xmax_, binning_, functname_.c_str(), poldegree_ ));
