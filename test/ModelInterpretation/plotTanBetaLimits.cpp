...
int plotTanBetaLimits(){
  ...
  string output_prefix = "13TeV_2018_Dec17-23";
  ...
  //ouptut folder
  string output = "/nfs/dust/cms/user/leyvaped/Analyses/MSSM/FullRun2/Combine/April_2023/CMSSW_11_3_4/src/Analysis/Combine/Run2018/Inputs_Unc/model-interpretation/Results/";

  //need? setFolderNamesAccordingToBlinding(path2016_solo,output);
  string boson = "both";

  std::vector<AvailableScenarios> scenarious = {MHMODP_200};//{MH125,NEGMU1,NEGMU2,NEGMU3};
  ...
  for(const auto& scenario : scenarious){
    ...
    limits.ReadCombineLimits(path2018_solo);
